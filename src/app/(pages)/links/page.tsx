        ...
        <div className="h-full flex flex-col items-center">
          <div className="flex flex-col items-center py-3 max-w-md w-full mt-10 space-y-3">
            ...
            {Links.map((link,index) =>(
              <div key={index} className="w-full hover:border-zinc-200 border-zinc-400 transition-all border py-3 px-3 mt-4 items-center gap-5 rounded-md flex bg-zinc-800/50">
                {link.icon}
                <Link className="cursor-none w-full text-center" href={link.href}>{link.title}</Link>
              </div>
            ))}
            </div>
          </div>
